...
func (r *router) buildCustomersRoutes() {
	...
	c := r.rg.Group("/customers")
	{
		...
		c.POST("", handler.Post())
		c.GET("/recovery", handler.GetRecovery())
		c.GET("/totalstatus", handler.GetTotal())
		c.GET("/customeractive", handler.GetActiveTotal())
	}
}
...
func (r *router) buildInvoicesRoutes() {
	...
	i := r.rg.Group("/invoices")
	{
		...
		i.POST("", handler.Post())
		i.GET("/repare", handler.GetRepareDB())
		i.GET("/recovery", handler.GetRecovery())
	}
}
...
func (r *router) buildProductsRoutes() {
	...
	p := r.rg.Group("/products")
	{
		...
		p.POST("", handler.Post())
		p.GET("/recovery", handler.GetRecovery())
		p.GET("/topproducts", handler.GetTotalTop())
	}
}
...
func (r *router) buildSalesRoutes() {
	...
	s := r.rg.Group("/sales")
	{
		...
		s.POST("", handler.Post())
		s.GET("/recovery", handler.GetRecovery())
	}
}
